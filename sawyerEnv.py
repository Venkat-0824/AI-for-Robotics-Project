...
class sawyerEnv(gym.Env):
	...
	def reset(self):
		...
		#p.loadURDF("./table/table.urdf", [1.1, 0.000000, -0.3], p.getQuaternionFromEuler([(math.pi/2), 0, (math.pi/2)]))
		...
		path = 'data/data/random_urdfs/082/082.urdf'
		# self.objectId = p.loadURDF(path+'cardboard_box/'+'cardboard_box.urdf', xpos, ypos, -0.1, orn[0], orn[1], orn[2], orn[3]) # line 98
		self.objectId = p.loadURDF(path, [0.95,0,-0.1],p.getQuaternionFromEuler([0,0,3.1415]))
